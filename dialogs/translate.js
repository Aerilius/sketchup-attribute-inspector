export class Translate {
    ...
    try {
      ...
      if (key in this.mapping) {
        ...
        if (strippedKey in this.mapping) {
          strippedValue = this.mapping[strippedKey]
  ...
  html (rootNode = document.body) {
    ...
    for (var i = 0; i < textNodes.length; i++) {
      ...
      var strippedKey = String(text).replace(/^(\n|\s|&nbsp;)+|(\n|\s|&nbsp;)+$/g, '')
      let strippedValue = get(strippedKey)
...
function getTextNodes (node, textNodes, nodesWithAttr) {
    if (node && node.nodeType === 1 && !EXCLUDED_TAGS_PATTERN.test(node.nodeName)) {
        if (node.getAttribute('title') !== null && node.getAttribute('title') !== '') {
          nodesWithAttr['title'].push(node)
        }
        if (node.getAttribute('placeholder') !== null && node.getAttribute('placeholder') !== '') {
          nodesWithAttr['placeholder'].push(node)
        }
        for (let i = 0; i < node.childNodes.length; i++) {
            let childNode = node.childNodes[i]
            if (childNode && childNode.nodeType === 3 && !EMPTY_STRING_PATTERN.test(childNode.nodeValue)) {
