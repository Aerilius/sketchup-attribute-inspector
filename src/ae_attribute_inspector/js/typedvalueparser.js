...
define([], function () {
    ...
    /**
     * @exports self as TypedValueParser
     */
    var self = {};
    ...
    var COMMA                = /^\s*,\s*/,
        ...
        LENGTH               = /^[-]?\d+(?:\.\d+)?\s*?(mm|cm|m|"|')/,
        FLOAT                = /^[-+]?\d+[\.,](?:\d+(?:e[+-]\d+)?)?/,
        INTEGER              = /^[-]?\d+/,
        //STRING    = /^\"((?:[^\"]|\\\")*)\"/,
        STRING               = /^"((?:[^"]|\\")*)"|'((?:[^']|\\')*)'/,
        //COLOR     = /^Color\((\d+),\s?(\d+),\s?(\d+)(?:,\s?(\d+))\)/,
        COLOR                = /^(?:Color|rgb|rgba)\((\d+),\s*?(\d+),\s*?(\d+)(?:,\s*?(\d+))?\)|^#((?:[0-9a-f]{2}){3,4})/,
        FLOAT_3TUPLE         = /^\s*?[\(\[]?[-+]?\d+(?:\.\d+(?:e[+-]\d+))?,\s*?[-+]?\d+(?:\.\d+(?:e[+-]\d+))?,\s*?[-+]?\d+(?:\.\d+(?:e[+-]\d+))?[\)\]]?\s*?$/,
        COLOR_34TUPLE        = /^\s*?[\(\[]?(\d+),\s*?(\d+),\s*?(\d+)(?:,\s*?(\d+))?[\)\]]?\s*?$/,
        POINT3D              = /^(?:Geom::)?Point3d\(([-]?\d+(?:\.\d+)?),\s?([-]?\d+(?:\.\d+)?),\s?([-]?\d+(?:\.\d+)?)\)/,
        VECTOR3D             = /^(?:Geom::)?Vector3d\(([-]?\d+(?:\.\d+)?),\s?([-]?\d+(?:\.\d+)?),\s?([-]?\d+(?:\.\d+)?)\)/,
        SQUARE_BRACKET_START = /^\[/,
        SQUARE_BRACKET_END   = /^]/,
        NONCOMMA             = /^[^,]+/;


    self.TYPES = ['String', 'JSON', 'Integer', 'Float', 'Length', 'Array', 'Boolean', 'NilClass', 'Geom::Point3d', 'Geom::Vector3d', 'Sketchup::Color', 'Time'];

    self.getDisplayName = function (type) {
     ...
     * @returns {boolean}
     */
    self.validate = function (string, type) {
        string = strip(string);
        switch (type) {
            case 'Boolean':
            ...
            case 'Time':
                return (!isNaN(new Date(string).getTime())) || TIME.test(string) && RegExp["$'"] == '';
            case 'Length':
            ...
            case 'Sketchup::Color':
                return (COLOR.test(string) || COLOR_34TUPLE.test(string)) && RegExp["$'"] == '';
            case 'Geom::Point3d':
                return (POINT3D.test(string) || FLOAT_3TUPLE.test(string)) && RegExp["$'"] == '';
            case 'Geom::Vector3d':
                return (VECTOR3D.test(string) || FLOAT_3TUPLE.test(string)) && RegExp["$'"] == '';
            case 'Array':
     ...
     * @returns {string}
     */
    self.canonicalizeString = function (string, type) {
        if (type == 'String') {
            ...
            case 'Sketchup::Color':
                if (COLOR.test(string) || COLOR_34TUPLE.test(string)) {
                    if (RegExp.$5 != '') {
                        ...
                        var result = [];
                        for (var i = 0; i < RegExp.$5.length; i += 2) {
                    ...
                    throw new Error("Function canonicalizeString attempted to exit with null when parsing: " + string);
                }
            case 'Geom::Point3d':
                if (POINT3D.test(string) || FLOAT_3TUPLE.test(string)) {
                    return 'Point3d(' + parseFloat(RegExp.$1) + ',' +
                        parseFloat(RegExp.$2) + ',' +
                        parseFloat(RegExp.$3) + ')';
                }
            case 'Geom::Vector3d':
                if (VECTOR3D.test(string) || FLOAT_3TUPLE.test(string)) {
                    return 'Vector3d(' + parseFloat(RegExp.$1) + ',' +
                        parseFloat(RegExp.$2) + ',' +
                        parseFloat(RegExp.$3) + ')';
                }
            default:
     ...
     * @returns {string}
     */
    self.dateToISO8601 = function (date) {
        // TODO: use date.toJSON();
        return ljust(date.getUTCFullYear(), 4) + '-' +
            ljust(date.getUTCMonth() + 1, 2) + '-' +
            ljust(date.getUTCDate(), 2) + 'T' +
            ljust(date.getUTCHours(), 2) + ':' +
            ljust(date.getUTCMinutes(), 2) + ':' +
            ljust(date.getUTCSeconds(), 2) + 'Z';
    };
    ...
    function canonicalizeStringStrict(string) {
        if (BOOLEAN.test(string)) {
            ...
            var float = parseFloat(RegExp['$&'].replace(/\,/, '.'));
            ...
            if (!isNaN(d.getTime())) {
                ...
                while (!SQUARE_BRACKET_END.test(RegExp["$'"]) && !RegExp["$'"] == '') {
                    COMMA.test(RegExp["$'"]);
                    var arrayElement = canonicalizeStringStrict(RegExp["$'"]);
